...
@section('righttopbar')
  <div class="col-md-12 col-xs-12" >
    ...
    <div class="dropdown dropdown-bottom" style="display:inline-block">
      ...
      <a href="{{asset('/usageChart')}}?faculty={{$faculty}}&start={{$start_date}}&end={{$end_date}}&q={{$q}}"><button class="btn btn-primary dropdown-button-topbar" >Chart การใช้งาน</button></a>
      ...
      <div class="col-sm-4 col-xs-12">
        ...
        <div id="custom-search-input">
              <div class="input-group">
                  <input name="q" type="text" class="form-control input-lg" value="{{$q}}" placeholder="ค้นหาตามชื่อ, นามสกุล, เบอร์โทรศัพท์, etc" />
  ...
  <div style="background:white" >
    ...
    <div class="table-responsive" style="padding:10px;">
      <table class="table" id="myTable" >
        <thead>
          <tr>
            <th onclick="window.location='{{asset('/')}}?sort_by=user_information.firstname&sorting=@if($default_sort == 'user_information.firstname'){{$sorting}}@else{{'desc'}}@endif&faculty={{$faculty}}&start={{$start_date}}&end={{$end_date}}&q={{$q}}'" @if($default_sort == 'user_information.firstname')@if($current_sorting == 'asc')class="sorting_asc"@else class="sorting_desc"@endif @else class="sorting"@endif>Firstname</th>
            <th onclick="window.location='{{asset('/')}}?sort_by=user_information.lastname&sorting=@if($default_sort == 'user_information.lastname'){{$sorting}}@else{{'desc'}}@endif&faculty={{$faculty}}&start={{$start_date}}&end={{$end_date}}&q={{$q}}'" @if($default_sort == 'user_information.lastname')@if($current_sorting == 'asc')class="sorting_asc"@else class="sorting_desc"@endif @else class="sorting"@endif>Lastname</th>
            <th onclick="window.location='{{asset('/')}}?sort_by=user_information.phone_no&sorting=@if($default_sort == 'user_information.phone_no'){{$sorting}}@else{{'desc'}}@endif&faculty={{$faculty}}&start={{$start_date}}&end={{$end_date}}&q={{$q}}'" @if($default_sort == 'user_information.phone_no')@if($current_sorting == 'asc')class="sorting_asc"@else class="sorting_desc"@endif @else class="sorting"@endif>Phone Number</th>
            <th onclick="window.location='{{asset('/')}}?sort_by=user_information.email&sorting=@if($default_sort == 'user_information.email'){{$sorting}}@else{{'desc'}}@endif&faculty={{$faculty}}&start={{$start_date}}&end={{$end_date}}&q={{$q}}'" @if($default_sort == 'user_information.email')@if($current_sorting == 'asc')class="sorting_asc"@else class="sorting_desc"@endif @else class="sorting"@endif>Email</th>
            <th onclick="window.location='{{asset('/')}}?sort_by=user_information.company_name&sorting=@if($default_sort == 'user_information.company_name'){{$sorting}}@else{{'desc'}}@endif&faculty={{$faculty}}&start={{$start_date}}&end={{$end_date}}&q={{$q}}'" @if($default_sort == 'user_information.company_name')@if($current_sorting == 'asc')class="sorting_asc"@else class="sorting_desc"@endif @else class="sorting"@endif>Company Name</th>
            <th onclick="window.location='{{asset('/')}}?sort_by=user_information.motive&sorting=@if($default_sort == 'user_information.motive'){{$sorting}}@else{{'desc'}}@endif&faculty={{$faculty}}&start={{$start_date}}&end={{$end_date}}&q={{$q}}'" @if($default_sort == 'user_information.motive')@if($current_sorting == 'asc')class="sorting_asc"@else class="sorting_desc"@endif @else class="sorting"@endif>Motive</th>
...
$('#pagination-demo').twbsPagination({
        ...
        onPageClick: function (event, page) {
            ...
            window.location = "{{asset('/?page=')}}"+String(page)+'&sort_by='+'{{$default_sort}}'+'&sorting='+'{{$current_sorting}}'+
                              '&faculty={{$faculty}}&start={{$start_date}}&end={{$end_date}}&q={{$q}}';
        }
    });
  });
  </script>
